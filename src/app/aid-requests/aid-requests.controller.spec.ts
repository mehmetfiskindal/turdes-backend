...
describe('AidRequestsController', () => {
  ...
  describe('findAll', () => {
    it('should return an array of aid requests', async () => {
      ...
      expect(
        await controller.findAll({ user: { id: 1 } } as RequestWithUser),
      ).toBe(result);
    });
  });
  ...
  describe('findOne', () => {
    it('should return a specific aid request', async () => {
      ...
      expect(
        await controller.findOne(1, 1, { user: { id: 1 } } as RequestWithUser),
      ).toBe(result);
    });
  });
  ...
  describe('updateStatus', () => {
    it('should update the status of a specific aid request', async () => {
      ...
      expect(await controller.updateStatus(1, 'Completed', '1', 'admin')).toBe(
        result,
      );
    });
  });
  ...
  describe('uploadDocument', () => {
    it('should upload a document to a specific aid request', async () => {
      ...
      expect(
        await controller.uploadDocument(1, 'Document', 'http://example.com'),
      ).toBe(result);
    });
  });
